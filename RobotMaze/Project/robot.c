...
int checkRobotSensorRightAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir =round(robotCentreX+(ROBOT_HEIGHT/2+SENSOR_VISION-sensorSensitivityLength*i)*cos((robot->angle)*PI/180));
        yDir = round(robotCentreY+(ROBOT_HEIGHT/2+SENSOR_VISION-sensorSensitivityLength*i)*sin((robot->angle)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void move(int direction,struct Robot * robot){
    switch(direction){
        case 1:
            robot->currentSpeed += DEFAULT_SPEED_CHANGE;
            if (robot->currentSpeed > MAX_ROBOT_SPEED)
...
void towardswall(struct Robot * robot, int left,int right){
    if(left>right){
        move(3,robot);
    }
    else if(right>left){
...
speeddown=0;
turnright=0;
int SATAG; //stop and turn and go
SATAG=0;
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor,int right_sensor) {
    if (turnleft){
        move(3,robot);
        counter=counter+1;
        if (counter==6){
        ...
        else if((front_left_sensor>0)&&(front_left_sensor==front_right_sensor)){
            turnleft=1;
        }
        ...
        else if((front_left_sensor >=3)&&(front_right_sensor>=3)){
            move(2,robot);
            towardswall(robot,front_left_sensor,front_right_sensor);
            turnleft=1;
            start_wall=1;
       }

    }
    else if ((front_left_sensor == 0) && (front_right_sensor == 0)  && ((right_sensor == 3))) {
        if (robot->currentSpeed<3){
            move(1,robot);
        }

    }
    else if((robot->currentSpeed>1) && (((front_left_sensor >=2) || (front_right_sensor >= 2))) ){
        if(robot->currentSpeed>1){
            move(2,robot);
        }

    }
    else if ((robot->currentSpeed>0) && ((front_left_sensor >= 3) && (front_right_sensor >= 3)) ) {  //if face a wall
        SATAG=1;
    }
    else if ((robot->currentSpeed>0)&&((right_sensor>=4)||(front_right_sensor>=3) )){
        move(3,robot);

    }  //if close to wall


    else if ((right_sensor<=2) ) {  // if far from wall
            if (robot->currentSpeed>3){
                move(2,robot);
            }
